...
void smithyTest(struct gameState* origG){
	...
	int result = cardEffect(smithy, 0, 0, 0, &newG, 0, &bonus);
	...
	//check if the 3 cards came from the right deck
	assertEqual(origG->deckCount[curPlayer]-3, newG.deckCount[curPlayer], DECK_COUNT_FAIL);
...
int main() {
    ...
    printf("RANDOM TESTING %d ITERATIONS OF smithy card:\n", TEST_ITERATIONS);
